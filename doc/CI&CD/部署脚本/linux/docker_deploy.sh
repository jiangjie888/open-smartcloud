...
#检查目录文件
if [ ! -f $MYAPPS_DIR/$PRJECT_NAME/Dockerfile ]; then
    echo $MYAPPS_DIR/$PRJECT_NAME/Dockerfile" is not exist"
    exit 1
fi

if [ ! -f $MYAPPS_DIR/$PRJECT_NAME/$PACKAGE ]; then
    echo $MYAPPS_DIR/$PRJECT_NAME/$PACKAGE" is not exist"
	exit 1
fi
...
#imageid=$(docker images | grep "$IMAGE_NAME" | awk '{print $1}')
#if [ "$cid" != "" ]; then
...
echo $MYAPPS_DIR/$PRJECT_NAME
cd $MYAPPS_DIR/$PRJECT_NAME
#创建镜像
echo "create docker image "$IMAGE_NAME:$LAST_TAG
docker build -t $IMAGE_NAME:$LAST_TAG .
#启动docker 容器
echo "start docker container "$CONTAINER_NAME
docker run -d -p $API_PORT:8888 --name $CONTAINER_NAME $IMAGE_NAME:$LAST_TAG
