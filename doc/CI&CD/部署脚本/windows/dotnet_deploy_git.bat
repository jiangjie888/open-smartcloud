...
set "PROJECT_DIR=%2"
set "PROJECT_ENV=%3"
set IIS_SITE=%PROJECTGROUP_DIR%_%PROJECT_ENV%
if "%PROJECTGROUP_DIR%"=="" (
...
@C:\Windows\System32\inetsrv\appcmd.exe stop apppool /apppool.name:"%IIS_SITE%"
...
WinRAR.exe x -y -o+ %SSH_DIR%\%PROJECTGROUP_DIR%\%PROJECT_DIR%.rar %DEPLOY_DIR%\%PROJECTGROUP_DIR%\%PROJECT_DIR%\
echo ============================end unrar to site dir=======================================
rem 启动IIS站点
@C:\Windows\System32\inetsrv\appcmd.exe start apppool /apppool.name:"%IIS_SITE%"
