...
import com.central.oauth.core.exception.ValidateCodeException;
import com.central.oauth.modular.consumer.UserServiceConsumer;
import com.central.oauth.modular.service.IValidateCodeService;
...
public class ValidateCodeServiceImpl implements IValidateCodeService {
    ...
    public ResponseData sendSmsCode(String mobile) {
        ...
        if (tempCode != null) {
            ...
            return ResponseData.error("验证码未失效，请失效后再次申请");
        }

        SysUser user = userServiceConsumer.findByMobile(mobile);
        if (user == null) {
            log.error("根据用户手机号{}查询用户为空", mobile);
            return ResponseData.error("手机号不存在");
        }
